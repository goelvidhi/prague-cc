...
For completeness, the pseudocode in \autoref{fig:prresp_per-ack-aimd} includes a Reno-like additive increase.
...
On_each_ACK'd_packet {
    // Update EWMA
    av_carry = div(av_up+av_carry.rem, flight*g);
    av_up += ce_fb - av_carry.quot;

    if (!ce_fb) {
        // Additive Increase
        cwnd_carry = div(g*2+cwnd_carry.rem, flight*g*2);
        cwnd += cwnd_carry.quot;
    } else if (!cwr) {
        // Record start of CWR state
        next_seq = snd_next;
    ...
    if (cwr) {
        ...
        if (snd_una < next_seq) {
            // Multiplicative Decrease
            _denom = flight*g*2;
            cwnd_carry.rem = _denom - cwnd_carry.rem;
            cwnd_carry = div(cwnd_carry.rem+av_up, _denom);
            cwnd_carry.rem = _denom - cwnd_carry.rem;
            cwnd -= cwnd_carry.quot;
        } else {
            cwr = false;
